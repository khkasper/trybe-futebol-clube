...
export default class Board {
  private static gamePoints(reaultA: number, resultB: number) {
    if (reaultA > resultB) return 3;
    if (reaultA < resultB) return 0;
    return 1;
  }

  private static points(match: IMatch, type: string) {
  ...
  private static victories(match: IMatch, type: string) {
    const points = Board.points(match, type);
    if (points === 3) return 1;
    return 0;
  }

  private static draws(match: IMatch, type: string) {
    const points = Board.points(match, type);
    if (points === 1) return 1;
    return 0;
  }

  private static losses(match: IMatch, type: string) {
    const points = Board.points(match, type);
    if (points === 0) return 1;
    return 0;
  }

  private static goalsFavor(match: IMatch, type: string) {
    if (type === 'home') return match.homeTeamGoals;
    return match.awayTeamGoals;
  }

  private static goalsOwn(match: IMatch, type: string) {
    if (type === 'home') return match.awayTeamGoals;
    return match.homeTeamGoals;
  }

  private static goalsBalance(match: IMatch, type: string) {
    if (type === 'home') return match.homeTeamGoals - match.awayTeamGoals;
    return match.awayTeamGoals - match.homeTeamGoals;
  }

  private static efficiency(points: number, games: number) {
  ...
  static update(match: IMatch, board: ILeaderboard, type: string) {
  ...
  static sort(board: ILeaderboard[]) {
